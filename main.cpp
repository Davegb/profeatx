...
void printEncoded(vector<string> headers, vector<vector<string>> encoded, int numSeqs, string output)
{
    ofstream file;
    file.open(output);
...
int main (int argc, char* argv[])
{
    ...
    // Read the file and create an array of sequences
    gzFile fp;
	kseq_t *seq;
	int l;
    int numSeqs;
    ...
    {
        Seq newSeq;
        string nameString(seq->name.s);
        string seqString(seq->seq.s);
    ...
    // Remove disallowed characters from sequences
    #pragma omp parallel for num_threads(numThreads) \
        default(none) shared(seqs, allowed, cout) schedule(guided) 
    ...
    if (encoding.compare("AAC") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = AAC(seqs[i], names[i], allowed);
            #pragma omp critical
    ...
    else if (encoding.compare("EAAC") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, window, windows) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = EAAC(seqs[i], names[i], allowed, keys, window, windows);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("CKSAAP") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, gap) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = CKSAAP(seqs[i], names[i], allowed, keys, gap);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("TPC") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = TPC(seqs[i], names[i], allowed, keys);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("DPC") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = DPC(seqs[i], names[i], allowed, keys);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("DDE") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, tm, tv) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = DDE(seqs[i], names[i], allowed, keys, tm, tv);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
        ...
        encoding.compare("GDPC") == 0 || encoding.compare("GTPC") == 0)
    {
        ...
        if(encoding.compare("GAAC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, keys, groups) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = GAAC(seqs[i], names[i], keys, groups);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("EGAAC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, groups, window, windows) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = EGAAC(seqs[i], names[i], keys, groups, window, windows);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("CKSAAGP") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, keys, gap, groups, groupStrings) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = CKSAAGP(seqs[i], names[i], keys, gap, groups, groupStrings);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("GDPC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, keys, groups, groupStrings) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = GDPC(seqs[i], names[i], keys, groups, groupStrings);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("GTPC") == 0)
        {
            ...
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = GTPC(seqs[i], names[i], keys, groups, groupStrings);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
    ...
    else if (encoding.compare("Binary") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = binary(seqs[i], names[i], keys);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("Moran") == 0 || encoding.compare("Geary") == 0 || encoding.compare("NMB") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, indexData, indexList, lag, means, order, encoding) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein;
            if (encoding.compare("Moran") == 0)
                encodedProtein = Moran(seqs[i], names[i], keys, indexData, indexList, lag, means, order);
            else if (encoding.compare("Geary") == 0)
                encodedProtein = Geary(seqs[i], names[i], keys, indexData, indexList, lag, means, order);
            else if (encoding.compare("NMB") == 0)
                encodedProtein = NMB(seqs[i], names[i], keys, indexData, indexList, lag, means, order);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.rfind("CTD", 0) == 0)
    {
        ...
        if (encoding.compare("CTDC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, groups, properties) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = CTDC(seqs[i], names[i], allowed, keys, groups, properties);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("CTDT") == 0)
        {
            ...
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = CTDT(seqs[i], names[i], allowed, keys, groups, properties);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("CTDD") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, groups, properties, pcts) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = CTDD(seqs[i], names[i], allowed, keys, groups, properties, pcts);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
    ...
    else if (encoding.compare("CT") == 0 || encoding.compare("KSCT") == 0)
    {
        ...
        if (encoding.compare("CT") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, groups) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = CT(seqs[i], names[i], allowed, keys, groups);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("KSCT") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, groups, k) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = KSCT(seqs[i], names[i], allowed, keys, groups, k);
                encoded.push_back(encodedProtein);
            }
        }
    }
    ...
    else if (encoding.compare("QSO") == 0 || encoding.compare("SOCN") == 0)
    {
        ...
        if (encoding.compare("SOCN") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, order, swData, gData, lag) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = SOCNumber(seqs[i], names[i], allowed, keys, order, swData, gData, lag);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("QSO") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, order, swData, gData, lag, weight) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = QSOrder(seqs[i], names[i], allowed, keys, order, swData, gData, lag, weight);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
    ...
    else if (encoding.compare("PAAC") == 0 || encoding.compare("APAAC") == 0)
    {
        ...
        if(encoding.compare("PAAC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, lag, order, paac, weight) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = PAAC(seqs[i], names[i], allowed, keys, lag, order, paac, weight);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
        else if (encoding.compare("APAAC") == 0)
        {
            ...
            #pragma omp parallel for num_threads(numThreads) \
                default(none) shared(encoded, seqs, names, allowed, keys, lag, order, paac, properties, weight) schedule(guided) 
            for (int i = 0; i < seqs.size(); i++)
            {
                vector<string> encodedProtein = APAAC(seqs[i], names[i], allowed, keys, lag, order, paac, properties, weight);
                #pragma omp critical
                encoded.push_back(encodedProtein);
            }
        }
    ...
    else if (encoding.compare("AAI") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, order, indexData, indexList) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = AAINDEX(seqs[i], names[i], keys, order, indexData, indexList);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("BLOSUM62") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, blosum) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = BLOSUM62(seqs[i], names[i], keys, blosum);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("ZS") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, zscale) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = ZSCALE(seqs[i], names[i], keys, zscale);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("SSEB") == 0 || encoding.compare("SSEC") == 0 || encoding.compare("SSPB") == 0 || encoding.compare("SSPAC") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, path, typesB, typesC, n, cout, encoding) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
    ...
    else if (encoding.compare("Disorder") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, path, cout) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            ifstream file;
            ...
            {
                vector<string> encodedProtein = Disorder(seqs[i], names[i], allowed, keys, file);
                #pragma omp critical
                encoded.push_back(encodedProtein);
                file.close();
            }
    ...
    else if (encoding.compare("DisorderC") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            ...
            {
                vector<string> encodedProtein = DisorderC(seqs[i], names[i], allowed, keys, file);
                #pragma omp critical
                encoded.push_back(encodedProtein);
                file.close();
            }
    ...
    else if (encoding.compare("DisorderB") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            ...
            {
                vector<string> encodedProtein = DisorderB(seqs[i], names[i], allowed, keys, file);
                #pragma omp critical
                encoded.push_back(encodedProtein);
                file.close();
            }
    ...
    else if (encoding.compare("TA") == 0 || encoding.compare("TAC") == 0 || encoding.compare("TAB") == 0 || encoding.compare("TAAC") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, n, path, cout, encoding) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            ifstream file;
    ...
    else if (encoding.compare("ASA") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            ...
            {
                vector<string> encodedProtein = ASA(seqs[i], names[i], allowed, keys, file);
                #pragma omp critical
                encoded.push_back(encodedProtein);
                file.close();
            }
    ...
    else if (encoding.compare("KNNprotein") == 0 || encoding.compare("KNNpeptide") == 0)
    {
        ...
        if (file.is_open())
        {
            ...
            kseq_t *train;
            int t;
            trainFile = gzopen(trainPath.c_str(), "r");
            train = kseq_init(trainFile);
            while ((t = kseq_read(train)) >= 0) 
            {
                Seq newSeq;
                string nameString(train->name.s);
                string seqString(train->seq.s);
                trainNames.push_back(nameString);
                trainSeqs.push_back(seqString);
                trainLen++;
            }
            file.close();
        }
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, keys, blosum, order, trainNames, trainSeqs, labelsData, labels, kNums, encoding) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein;
            if (encoding.compare("KNNpeptide") == 0)
    ...
    else if (encoding.compare("AAPAS") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, freqs) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = AAPAS(seqs[i], names[i], allowed, keys, freqs);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("TVD") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, tvd) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = TVD(seqs[i], names[i], allowed, keys, tvd);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("CMV") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = CMV(seqs[i], names[i], allowed, keys);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    else if (encoding.compare("EBGW") == 0)
    {
        ...
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = EBGW(seqs[i], names[i], allowed, keys, groups, k);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
        ...
        encoding.compare("PSSMAC") == 0 || encoding.compare("PPSSM") == 0)
    {
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, orderString, path, cout, encoding, n) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            ifstream file;
            ...
            else if (file.is_open())
            {
                vector<string> encodedProtein;
                if (encoding.compare("PSSM") == 0)
                    encodedProtein = PSSM(seqs[i], names[i], allowed, keys, file);
                else if (encoding.compare("PSSMAAC") == 0)
    ...
    {        
        ...
        #pragma omp parallel for num_threads(numThreads) \
            default(none) shared(encoded, seqs, names, allowed, keys, aaMap, ktuple, subtype, gapLambda) schedule(guided) 
        for (int i = 0; i < seqs.size(); i++)
        {
            vector<string> encodedProtein = PseKRAAC(seqs[i], names[i], allowed, keys, aaMap, ktuple, subtype, gapLambda);
            #pragma omp critical
            encoded.push_back(encodedProtein);
        }
    }
    ...
    printEncoded(headers, encoded, encoded.size(), output);
